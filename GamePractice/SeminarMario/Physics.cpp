...
JumpPhysics::JumpPhysics(int horizontalVelocity, int initialVerticalVelocity, int gravity)
	:_gravity(gravity), _initialJumpVelocity(horizontalVelocity, -initialVerticalVelocity)
{
}
...
void BoundedPhysicsDecorator::test()
{
	if (_base->getTL().x > _bounds.x + _bounds.width - _base->getCollisionMask().size().width || _base->getTL().x < 0) {
		...
		_base->reset(Point(_bounds.width/100, _bounds.height * 3 / 4));
}
	//_bounds.contains() ;
	//< _bounds.x + _bounds.width - _base->getCollisionMask().size().width;
}
...
BoundedPhysicsDecorator::BoundedPhysicsDecorator(IPhysicsComponentPtr base) :_base(base)
{
	...
	_bounds.x = 0;
	_bounds.y = 0;
	_bounds.width = background.size().width - _base->getCollisionMask().size().width;
	_bounds.height = background.size().height - _base->getCollisionMask().size().height;
