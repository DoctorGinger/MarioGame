...
int main()
{
	...
	slimeState->reset(Point(background.size().width * 2 / 4, background.size().height * 5 / 6));
	
	auto slimenemy =createSlime(R"(../Animations/SlimeOrange)");
	slimenemy->reset(Point(background.size().width * 2 / 3, background.size().height * 5/6));

	EntityPtr hero = createHero(R"(../Animations/Hero)");
	hero->reset(Point(background.size().width /9, background.size().height * 3 / 4));
	

	EntityPtr slime(new Entity(slimeState));
	Timer timer(/*freq. ms=*/100);
	timer.Register(slime);
	timer.Register(slimenemy);
	timer.Register(hero);
